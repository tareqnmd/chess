...
const GameBoard = ({
	...
	const boardOrientation =
		gameState.settings?.playerColor === 'b' ? 'black' : 'white';
	const isGameOver =
		gameState.status !== 'playing' && gameState.status !== 'idle';
	const canInteract =
		!disabled && !isGameOver && isPlayerTurn && gameState.status === 'playing';
	...
	const onSquareClick = useCallback(
		({ square }: { square: string; piece?: string }) => {
			...
			if (
				!moveFrom &&
				clickedPiece &&
				clickedPiece.color === gameState.settings.playerColor
			) {
